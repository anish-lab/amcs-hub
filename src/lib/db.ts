...
export const DEFAULT_TIMETABLE_HTML = `
...
// Guarantees student profile availability across Vercel serverless containers
export async function getStudentProfile(rollNo: string) {
  ...
  // If running in Vercel serverless container and profile is missing, initialize profile automatically
  if (!student) {
    try {
      ...
      const newProfile = await prisma.studentProfile.create({
        data: {
          ...
          batchId: batch.id,
          semesterId: sem.id,
          timetableHtml: DEFAULT_TIMETABLE_HTML,
        }
      });
      ...
      console.error("Error initializing student profile:", e);
    }
  }
